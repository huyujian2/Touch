 ...
 # ********************************************************************************************#
    ...
    def GetSensorStatus(self,ofWhatSensor):
        ...
        # Z-axis Sensor
            ...
            command = '%01#RCSX000F'
        elif (ofWhatSensor == self.USB1Sensor):
            command = '%01#RCSX0300'
        elif (ofWhatSensor == self.USB2Sensor):
            command = '%01#RCSX0301'
        elif (ofWhatSensor == self.USB3Sensor):
            command = '%01#RCSX0302'
        elif (ofWhatSensor == self.USB4Sensor):
            command = '%01#RCSX0303'
...
#***********************************************************************************************#
    ...
    # ********************************************************************************************#
    def BojayFFTStart(self,DUT1,DUT2,DUT3,DUT4,DUTALL):
        try:
            if(DUTALL == True):
                if (self.GetSensorStatus(self.USBALLSensor) == 1):
                    err = self.DUTLcokOrOpen(self.DUTALL, self.DUT_OPEN)
                    if (err == -1):
                        return -1
            else:
                #:0:Check USB sensor
                if(DUT1==True):
                    if(self.GetSensorStatus(self.USB1Sensor) == 1):
                        err = self.DUTLcokOrOpen(self.DUT1,self.DUT_OPEN)
                        if (err == -1):
                            return -1
                if (DUT2 == True):
                    if (self.GetSensorStatus(self.USB2Sensor) == 1):
                        err = self.DUTLcokOrOpen(self.DUT2, self.DUT_OPEN)
                        if (err == -1):
                            return -1
                if (DUT3 == True):
                    if (self.GetSensorStatus(self.USB3Sensor) == 1):
                        err = self.DUTLcokOrOpen(self.DUT3, self.DUT_OPEN)
                        if (err == -1):
                            return -1
                if (DUT4 == True):
                    if (self.GetSensorStatus(self.USB4Sensor) == 1):
                        err = self.DUTLcokOrOpen(self.DUT4, self.DUT_OPEN)
                        if (err == -1):
                            return -1
...
#stop by ludy 0318
            ...
            if(self.bFirstRunFunction == True):
                ...
                for line in alllines:
                    ...
                    if("X-Finial=" in strLine):
                        ...
                        self.ZAxisCalibration = float(strLine[strLine.find("=")+1:])
                    elif("z-original=" in strLine):
                        self.ZAxisOriginal = float(strLine[strLine.find("=")+1:])
            ...
            self.strErrorMessage = "BojayFFTStart error"
            return -1
 ...
 # ********************************************************************************************#
    def CalibratePositionOfV71(self, point):
        try:
            ...
            isTouched = False
            calPoints = []
            originalPoints = [[25,65],[85,65],[55,25],[55,105]]
            axises = [self.X_axis,self.X_axis,self.Y_axis,self.Y_axis]
            ...
            for i in range(0,4):
                ...
                time.sleep(2)
                #Push Z axis down
                if(self.MoveToCoordinates(self.Z_axis, 20, 5) == -1):
                    return -1
                time.sleep(1)
                ...
                xPosition = self.GetCurrentCoordinates(self.X_axis)
                yPosition = self.GetCurrentCoordinates(self.Y_axis)
            ...
            point.append = (calPoints[0][0] + calPoints[1][0])
            point.append = (calPoints[2][1] + calPoints[3][1])
            filepath = os.getcwd()
