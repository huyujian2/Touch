...
1.add function(FixtureStatus): get the curtain sensor status:
    return 1  -->  Fixture normal
    return -1 -->  Fixture status except fail
    return -2 -->  Something touches the curtain
    return -3 -->  has puss EStop
2.when execute API:MoveToCoordinates,it will catch fixture status at the same time. 
    return 0  -->  execute MoveToCoordinates successfully
    return -1 -->  execute MoveToCoordinates time out
    return -2 -->  Something touches the curtain
    return -3 -->  has puss EStop
'''
...
class GOEControlClass:
    ...
    def MoveToCoordinates(self,ofWhatAxis,Value,timeout=10):
        try:
            ...
            mytimeCount = 0
            while (self.GetmoveSignal(ofWhatAxis) == 1 && self.FixtureStatus() == 1):
                if self.FixtureStatus == False:
                    return -2
                time.sleep(0.005)
